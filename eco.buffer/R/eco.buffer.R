...
#' @param timing display timing messages if TRUE (verbose must be TRUE too).
#' @details
#' The ecological buffer tool **eco.buffer** is a stand-alone R package for delineating
#' ecologically-defined
#' buffers around target conservation areas. Targeted areas are designated by
#' point, line, or polygon shapefiles. Buffers are based on resistant kernels with
#' flexible parameters to accommodate terrestrial or aquatic settings. Landscape
#' resistance can be defined by a landcover raster and a table of classes and resistance
#' values, directly from resistance rasters, or from a combination of a resistance table
#' and rasters. Results are a polygon shapefile of ecological buffers and an optional
#' geoTIFF raster of the resistant kernels.
#'
...
# B. Compton, 2 Apr 2021-29 May 2021
...
'eco.buffer' <- function(seeds, bandwidth, seedid = 'Id', result = NULL, resultgrid = NULL, resist = NULL,
                         ...
                         expand = 0, screen = NULL, broccoli = NULL, flow = NULL, accumulation = NULL,
   ...
   if(!is.null(broccoli)) stop('broccoli option is not yet implemented')
   ...
   streams <- check.file(streams, path, 'Streams grid', verbose, require = !is.null(broccoli))
   flow <- check.file(flow, path, 'Flow grid', verbose, require = !is.null(broccoli))
   accumulation <- check.file(accumulation, path, 'Flow accumulation grid', verbose, require = !is.null(broccoli))
   ...
   # Read vector data    # ------------------------ READ AND CLIP DATA ------------------------
   ...
   chatter(verbose, 'Getting resistance values...')  # ------------------------ BUILD RESISTANCE GRID ------------------------
   t <- proc.time()[3]
   ...
   # Loop through landscape and build kernels   # ------------------------ BUILD RESISTANT KERNELS ------------------------
   ...
   if(!is.null(broccoli)) {  # if broccoli option, build broccoli watersheds    # ------------------------ BROCCOLI ------------------------

      y<<-y;w<<-w;flowg<<-flowg;streamg<<-streamg;accumg<<-accumg;z<<-z;return()

      q <- broccoli(y, w, streams, flow, accumulation)
      y <- q[[1]]
      w <- q[[2]]
   }
   ...
   # Write results
   chatter(verbose, 'Saving results...')   # ------------------------ SAVE RESULTS ------------------------
   if(!is.null(result)) {
      z[] <- w
      ...
      write.shapefile(p, result, 'result', verbose, timing)
   }

   if(!is.null(resultgrid))
      write.tiff(y, z, resultgrid, 'result kernel', verbose, timing)

   if(!is.null(save.resist))
      write.tiff(r, z, save.resist, 'saved resistance', verbose, timing)

   chatter(timing & verbose, '  Total elapsed time = ', proc.time()[3] - t0, ' s')
}
